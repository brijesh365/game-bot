...
from google_search.auth import environment_vars
from google_search.auth import exceptions
import google_search.auth.transport._http_client
...
def _warn_about_problematic_credentials(credentials):
    ...
    """
    from google_search.auth import _cloud_sdk
...
def _load_credentials_from_file(filename):
    ...
    if credential_type == _AUTHORIZED_USER_TYPE:
        ...
        from google_search.oauth2 import service_account
...
def _get_gae_credentials():
    ...
    try:
        import google_search.auth.app_engine as app_engine
    except ImportError:
        return None, None
...
def _get_gce_credentials(request=None):
    ...
    # some cases where it's not available, so we tolerate ImportError.
    try:
        from google_search.auth import compute_engine
        from google_search.auth.compute_engine import _metadata
    except ImportError:
        return None, None

    if request is None:
        request = google_search.auth.transport._http_client.Request()
...
def default(scopes=None, request=None):
    ...
    Raises:
        ~google.auth.exceptions.DefaultCredentialsError:
            ...
            invalid.
    """
    from google_search.auth.credentials import with_scopes_if_required
